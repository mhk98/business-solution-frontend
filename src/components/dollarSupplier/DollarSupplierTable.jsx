...
} from "../../features/dollarSupplier/dollarSupplier";
...
const DollarSupplierTable = () => {
  ...
  const { currentData: summaryData, isFetching: summaryFetching } =
    useGetAllDollarSupplierWithoutQueryQuery(dateParams, {
      pollingInterval: 3000,
      refetchOnFocus: true,
      refetchOnMountOrArgChange: true,
      refetchOnReconnect: true,
  ...
  useEffect(() => {
    if (isError) {
      console.error("Error fetching dollar supplier data", error);
      return;
    }
  ...
  const [insertDollarSupplier] = useInsertDollarSupplierMutation();
  const handleCreateSupplier = async (e) => {
    ...
    try {
      ...
      const res = await insertDollarSupplier(payload).unwrap();

      if (res?.success) {
        toast.success("Dollar Supplier created successfully!");
  ...
  const [updateDollarSupplier] = useUpdateDollarSupplierMutation();
  const handleUpdateSupplier = async () => {
    if (!currentProduct?.Id)
      return toast.error("Invalid dollar supplier selected!");
    ...
    try {
      ...
      const res = await updateDollarSupplier({
        id: currentProduct.Id,
        data: updated,
      }).unwrap();

      if (res?.success) {
        toast.success("Dollar Supplier updated successfully!");
  ...
  const [deleteDollarSupplier] = useDeleteDollarSupplierMutation();
  const handleDeleteSupplier = async (id) => {
    const confirmDelete = await requestDeleteConfirmation({
      message: "Do you want to delete this dollar supplier?",
    });
    if (!confirmDelete) return toast.info("Delete action was cancelled.");

    try {
      const res = await deleteDollarSupplier(id).unwrap();
      if (res?.success !== false) {
        toast.success("Dollar Supplier deleted successfully!");
        refetch?.();
              ...
              <div className="relative flex items-start justify-between">
                <div>
                  ...
                  <p className="mt-1 text-[11px] text-slate-400">
                    Dollar supplier payment completed
                  </p>
            ...
            <div className="group relative overflow-hidden rounded-2xl border border-sky-200 bg-white p-5 shadow-sm transition hover:shadow-md">
              ...
              <div className="relative flex items-start justify-between">
                <div>
                  ...
                  <p className="mt-1 text-[11px] text-slate-400">
                    Extra paid to dollar suppliers
                  </p>
            ...
            <div className="group relative overflow-hidden rounded-2xl border border-rose-200 bg-white p-5 shadow-sm transition hover:shadow-md">
              ...
              <div className="relative flex items-start justify-between">
                <div>
                  ...
                  <p className="mt-1 text-[11px] text-slate-400">
                    Dollar supplier amount still due
                  </p>
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div className="relative w-full sm:max-w-[520px]">
          <input
            ...
            placeholder="Search dollar supplier..."
        ...
        <div className="flex items-center gap-3">
          <div className="flex items-center gap-2">
            <label
              htmlFor="dollarSupplierPerPage"
              className="text-xs font-semibold uppercase tracking-wide text-slate-500"
            >
              Per Page
            </label>
            <select
              id="dollarSupplierPerPage"
              value={itemsPerPage}
              ...
              {suppliers.map((item) => (
                <tr key={item.Id} className="hover:bg-slate-50 transition">
                  <td className="px-5 py-4">
                    <Link
                      to={`/dollar-supplier-history/${item.Id}`}
        ...
        <div className="space-y-4">
          <div>
            ...
            <input
              ...
              placeholder="Enter dollar supplier name"
            />
          </div>
...
export default DollarSupplierTable;
