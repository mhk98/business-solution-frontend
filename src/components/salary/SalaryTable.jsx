...
const PAYROLL_FINE_FIELDS = [
  {
    label: "Late Penalty (3 Days)",
    field: "late",
    icon: Clock,
  },
  {
    label: "Early Leave Penalty (3 Days)",
    field: "early_leave",
    icon: UserMinus,
  },
  {
    label: "Standard Absent ( 1 Day)",
    field: "absent",
    icon: CalendarX,
  },
  {
    label: "Friday Absent (1 Day)",
    field: "friday_absent",
    icon: AlertCircle,
  },
  {
    label: "Unauthorized Absence (1 Day)",
    field: "unapproval_absent",
    icon: AlertCircle,
  },
];
  ...
  const handleDeleteProduct = async (id) => {
    if (
      await requestDeleteConfirmation({
        message: "Delete these payroll fine parameters?",
      })
    ) {
      try {
  ...
  return (
    ...
    >
            ...
            <tbody className="divide-y divide-slate-100 bg-white">
              {products ? (
                <>
                  {[
                    {
                      label: t.late_arrival_threshold,
                      value: products.late,
                      icon: Clock,
                    ...
                    {
                      label: t.early_departure_limit,
                      value: products.early_leave,
                      icon: UserMinus,
                      color: "text-blue-500",
                      bg: "bg-blue-50",
                    },
                    {
                      label: t.standard_absenteeism,
                      value: products.absent,
                      icon: CalendarX,
                      color: "text-rose-500",
                      bg: "bg-rose-50",
                    },
                    {
                      label: t.unscheduled_friday_absence,
                      value: products.friday_absent,
                      icon: AlertCircle,
                      color: "text-purple-500",
                      bg: "bg-purple-50",
                    },
                    {
                      label: t.unauthorized_leave,
                      value: products.unapproval_absent,
                      icon: AlertCircle,
                      color: "text-red-600",
                      bg: "bg-red-50",
                    },
                    ...
                    >
                      <td className="px-6 py-5 whitespace-nowrap">
                        <div className="flex items-center gap-3">
                          <div
                            className={`h-10 w-10 flex items-center justify-center rounded-xl ${item.bg} ${item.color} shadow-sm border border-slate-100 group-hover:scale-110 transition-transform`}
                          >
                            <item.icon size={18} />
                          </div>
                      ...
                      {idx === 0 && (
                        <td
                          rowSpan={5}
                          className="px-6 py-5 whitespace-nowrap text-center align-middle border-l border-slate-50"
                        >
                ...
                <tr>
                  <td colSpan={3} className="py-24 text-center text-slate-400">
                    ...
                    <p className="font-bold text-sm italic">
                      Payroll parameters not initialized
                    </p>
                  </td>
                </tr>
              )}
      ...
      <Modal
        ...
        title="Update Payroll Logic"
      >
        {currentProduct && (
          <form onSubmit={handleUpdateProduct} className="space-y-6">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
              {PAYROLL_FINE_FIELDS.map((field, i) => (
                <div key={i} className="space-y-2">
                  <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1 flex items-center gap-1.5">
                    <field.icon size={12} className="text-indigo-500" />{" "}
                    {field.label}
                  </label>
                  <p className="ml-1 text-[11px] font-semibold leading-4 text-slate-500">
                    {field.note}
                  </p>
                  <input
                    type="number"
      ...
      <Modal
        ...
        title="Global Payroll Initialization"
      >
