...
import {
  ...
  isSmsNotificationPermissionKey,
...
const SmsNotificationPermissionsManager = () => {
  ...
  const selectedSmsNotificationKeys = useMemo(
    () => selectedRoleKeys.filter(isSmsNotificationPermissionKey),
    [selectedRoleKeys],
  );

  const updateSelectedRoleSmsKeys = (nextSmsKeys) => {
    const otherKeys = selectedRoleKeys.filter(
      (key) => !isSmsNotificationPermissionKey(key),
    );
  ...
  const toggleSmsNotificationPermission = (permissionKey, checked) => {
    const currentSmsKeys = new Set(selectedSmsNotificationKeys);

    if (checked) currentSmsKeys.add(permissionKey);
    else currentSmsKeys.delete(permissionKey);

    updateSelectedRoleSmsKeys(Array.from(currentSmsKeys));
  };
  ...
  const handleSavePermissions = async () => {
    try {
      ...
      toast.success(`${selectedPermissionRole} SMS permissions updated.`);
    } catch (err) {
      toast.error(err?.data?.message || "Failed to update SMS permissions.");
    }
  };

  const handleClearSmsPermissions = () => {
    updateSelectedRoleSmsKeys([]);
  };

  if (!canManagePermissions) {
    return (
      <SettingSection icon={MessageSquareText} title="SMS Notification Permissions">
        <div className="rounded-2xl border border-amber-200 bg-amber-50 px-4 py-4 text-sm font-medium text-amber-800">
          You do not have access to manage SMS notification permissions.
        </div>
      </SettingSection>
    );
  }
  ...
  return (
    <SettingSection icon={MessageSquareText} title="SMS Notification Permissions">
      <div className="flex items-start justify-between gap-4 flex-wrap">
        <div>
          <h3 className="text-lg font-semibold text-slate-900">
            SMS Notification Permission Manager
          </h3>
          <p className="mt-1 text-sm text-slate-500">
            কোন role কোন menu/submenu-এর SMS notification পাবে সেটা এখান
            থেকে control করা যাবে।
          </p>
        </div>
      ...
      <div className="mt-6 grid gap-3 md:grid-cols-2 xl:grid-cols-3">
        {SMS_NOTIFICATION_ITEMS.map((item) => (
          <label
          ...
          >
            <input
              type="checkbox"
              checked={selectedSmsNotificationKeys.includes(
                item.permissionKey,
              )}
              onChange={(event) =>
                toggleSmsNotificationPermission(
                  item.permissionKey,
      ...
      <div className="mt-6 flex items-center justify-between gap-3 flex-wrap border-t border-slate-100 pt-5">
        <p className="text-xs text-slate-500">
          SMS notification allowed: {selectedSmsNotificationKeys.length}
        </p>

        <div className="flex items-center gap-3">
          <button
            type="button"
            onClick={handleClearSmsPermissions}
...
export default SmsNotificationPermissionsManager;
