...
const PackagingFactoryStockTable = () => {
  ...
  const { data, isLoading } = useGetAllPackagingFactoryStockQuery({
    page,
    limit,
  ...
  return (
    <div className="bg-white/90 rounded-3xl p-4 sm:p-8 border border-slate-100 shadow-sm">
      <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6 mb-10">
        <div>
          <h2 className="text-2xl font-black text-slate-900">
            Packaging Factory Stock
          </h2>
          <p className="text-slate-500 text-sm mt-1 font-medium">
            Manufacturer-wise packaging factory stock.
          </p>
        </div>
        <div className="inline-flex items-center gap-4 bg-indigo-50 border border-indigo-100 px-6 py-3 rounded-2xl">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4 mb-8 bg-slate-50/50 p-6 rounded-3xl border border-slate-100 items-end">
        ...
        <Select
          options={itemOptions}
          value={itemOptions.find((o) => o.value === name) || null}
      ...
      <div className="overflow-hidden rounded-3xl border border-slate-100">
          ...
          <tbody className="divide-y divide-slate-100">
            {rows.map((row) => (
              <tr key={row.Id}>
                ...
                <td className="px-6 py-5 text-sm text-slate-600">
                  {Number(row.unitValue || 0).toLocaleString()}
                </td>
                <td className="px-6 py-5 text-sm text-slate-600">
                  {row.unit || "Pcs"}
                </td>
        ...
        {!isLoading && rows.length === 0 && (
          <div className="py-16 text-center text-slate-400 font-bold">
            No packaging factory stock found.
          </div>
        )}
      </div>
