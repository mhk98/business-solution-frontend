...
} from "../../features/courierNoEntry/courierNoEntry";
...
const courierStatusOptions = ["On the way", "Received"].map((status) => ({
  value: status,
  label: status,
}));

const normalizeCourierStatus = (status) =>
  String(status || "").toLowerCase() === "received" ? "Received" : "On the way";

const statusOptions = ["Active", "Approved", "Pending"].map((status) => ({
  value: status,
  label: status,
}));
...
const getInventoryVariantColorOptions = (inventoryItem) => {
  const variants = getSelectableVariantRows(inventoryItem);
  return [...new Set(variants.map((v) => v.color).filter(Boolean))].map(
    (v) => ({ value: v, label: v }),
  );
};
...
const CourierNoEntryTable = () => {
  ...
  const selectedBulkAddInventoryItem = useMemo(
    () =>
      receivedData.find(
        (r) => String(r.Id) === String(bulkAddForm?.receivedId),
      ),
  ...
  const selectedEditInventoryItem = useMemo(
    () =>
      receivedData.find(
        (r) =>
          String(r.Id) ===
          String(currentItem?.receivedId || currentItem?.productId || ""),
      ),
  ...
  const createItemsTotalPurchase = useMemo(
    () =>
      createItems.reduce(
        (total, item) => total + (Number(item?.payload?.purchase_price) || 0),
        0,
      ),
    [createItems],
  );
  const createItemsTotalSale = useMemo(
    () =>
      createItems.reduce(
        (total, item) => total + (Number(item?.payload?.sale_price) || 0),
        0,
      ),
    [createItems],
  );
  ...
  const { data, isLoading, isError, error, refetch } =
    useGetAllCourierNoEntryQuery(queryArgs);

  useEffect(() => {
    if (isError) console.error("Courier No Entry fetch error:", error);
    if (!isLoading && data) {
      setRows(data?.data ?? []);
      setTotalPages(
  ...
  const buildBulkAddItem = () => {
    ...
    if (!String(bulkAddForm.courierNo || "").trim()) {
      return { error: "Please enter courier no" };
    }
    ...
    const inventoryVariants = getVariantDisplayRows(
      selectedBulkAddInventoryItem,
    );
    ...
    return {
      item: {
        ...
        courierNo: bulkAddForm.courierNo,
        name:
  ...
  const openEdit = (rp) => {
    ...
    setCurrentItem({
      ...
      note: rp.note ?? "",
      courierNo: rp.courierNo ?? "",
      status: rp.status ?? "",
      courierStatus: normalizeCourierStatus(rp.courierStatus),
  ...
  // mutations
  const [insertCourierNoEntry] = useInsertCourierNoEntryMutation();
  const [updateCourierNoEntry] = useUpdateCourierNoEntryMutation();
  const [deleteCourierNoEntry] = useDeleteCourierNoEntryMutation();
  ...
  const applyCreateGlobalFields = (payload) => ({
    ...
    courierStatus: normalizeCourierStatus(createForm.courierStatus),
  ...
  const buildCreatePayload = () => {
    ...
    const unitPurchasePrice =
      Number(selectedCreateInventoryItem?.purchase_price) || 0;
    ...
    const totalPurchasePrice =
      variantsPayload.length > 0 && inventoryVariants.length > 0
        ? variantsPayload.reduce((sum, v) => {
            ...
            const vUnitPrice =
              Number(invVariant?.purchase_price) || unitPurchasePrice;
      ...
      return prev.map((item, index) => {
        ...
        return {
          ...
          payload: {
            ...
            courierNo:
              item.payload?.courierNo || incomingItem.payload?.courierNo || "",
    ...
    setCreateItems((prev) =>
      prev.map((item, currentItemIndex) => {
        ...
        const invItem = receivedData.find(
          (r) =>
            Number(r.Id) ===
            Number(item.payload?.receivedId || item.payload?.productId),
        );
  ...
  const handleCreate = async (e) => {
    ...
    if (items.some((item) => !String(item.courierNo || "").trim())) {
      return toast.error("Please enter courier no for every product");
    }
    try {
      const payload = items.length === 1 ? items[0] : { items };
      const res = await insertCourierNoEntry(payload).unwrap();
      if (res?.success) {
  ...
  const handleUpdate = async () => {
    ...
    if (
      bulkItems.length &&
      bulkItems.some((item) => !String(item.courierNo || "").trim())
    ) {
      return toast.error("Please enter courier no for every product");
    }
    ...
    try {
      const payload =
        bulkItems.length > 0
          ? {
              ...
              courierStatus: normalizeCourierStatus(currentItem.courierStatus),
              warehouseId: Number(currentItem.warehouseId),
              courierNo: currentItem.courierNo,
      ...
      const res = await updateCourierNoEntry({
        id: currentItem.Id,
        data: payload,
      }).unwrap();
  ...
  const handleCourierStatusChange = async (row, nextStatus) => {
    if (!row?.Id) return toast.error("Invalid item");
    const normalizedStatus = normalizeCourierStatus(nextStatus);
    if (normalizeCourierStatus(row.courierStatus) === normalizedStatus) return;

    const rowItems = parseTransitItems(row.items);
    const payload =
      rowItems.length > 0
        ? {
            items: rowItems.map((item) => ({
              ...item,
              quantity: getTransitItemQuantity(item),
              courierNo: item.courierNo || row.courierNo,
            })),
            note: row.note,
            status: row.status || "Active",
            courierStatus: normalizedStatus,
            warehouseId: Number(row.warehouseId || row.warehouse?.Id || 0),
        ...
        : {
            ...
            variants: getVariantDisplayRows(row),
            receivedId: Number(row.receivedId || row.productId),
            productId: Number(row.productId || row.receivedId),
            userId,
            actorRole: role,
          };
  ...
  const handleDelete = async (id) => {
    const confirmed = await requestDeleteConfirmation({
      title: "Delete courier no entry?",
      message:
        "This courier no entry record will be removed permanently. This action cannot be undone.",
    });
    if (!confirmed) return;

    try {
      const res = await deleteCourierNoEntry(id).unwrap();
  ...
  return (
    ...
    >
      ...
      <div className="my-2 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        ...
        <div className="flex items-center justify-between sm:justify-end gap-3 rounded-xl border border-slate-200 bg-white px-4 py-2 shadow-sm">
          <div className="flex items-center gap-2 text-slate-700">
            ...
            <span className="text-sm">Total Courier Entry</span>
          </div>
      ...
      {/* Table */}
      <div className="courier-no-entry-table-scroll four-row-table-scroll mt-6 rounded-2xl border border-slate-200">
        <table className="w-full min-w-[2200px] divide-y divide-slate-200 [&_th]:px-3 lg:[&_th]:px-4 [&_td]:px-3 lg:[&_td]:px-4 [&_th]:py-3 [&_td]:py-3">
          ...
          <tbody className="divide-y divide-slate-200 bg-white">
            {rows.map((rp) => {
                    ...
                    return (
                      ...
                      variants.reduce(
                        (variantTotal, variant) =>
                          variantTotal +
                          getVariantUnitPrice(item, variant, "purchase_price") *
              ...
              const totalBuy =
                computedTotalBuy || Number(rp.purchase_price || 0);
              ...
              return (
                ...
                >
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-slate-700">
                    <select
                      value={normalizeCourierStatus(rp.courierStatus)}
                      onChange={(event) =>
                        handleCourierStatusChange(rp, event.target.value)
                      }
                      disabled={!canUpdateStatus}
                      className={`h-9 min-w-[132px] rounded-xl border px-3 text-xs font-semibold outline-none transition focus:ring-4 focus:ring-indigo-500/10 disabled:cursor-not-allowed disabled:opacity-70 ${
                        normalizeCourierStatus(rp.courierStatus) === "Received"
      ...
      >
        {currentItem && (
          <>
            {isEditingBulkTransit && (
              <div className="rounded-2xl border border-slate-200 bg-slate-50/60 p-4">
                  ...
                  <table className="min-w-[1020px] w-full text-sm">
                    <thead className="bg-slate-50 text-xs font-bold uppercase tracking-wider text-slate-500">
                      <tr>
                        ...
                        <th className="px-3 py-3 text-left">Courier No</th>
                        ...
                        >
                          ...
                          <td className="px-3 py-3 align-top">
                            <input
                              ...
                              className="h-10 w-full min-w-[160px] rounded-xl border border-slate-200 bg-white px-3 text-sm font-semibold text-slate-900 outline-none focus:border-indigo-300 focus:ring-4 focus:ring-indigo-500/10"
                ...
                <div className="mt-4 rounded-xl border border-indigo-100 bg-white p-3">
                  <div className="grid grid-cols-1 gap-3 lg:grid-cols-[minmax(220px,1fr)_180px_140px_auto] lg:items-end">
                    <div>
                    ...
                    <div>
                      ...
                      <input
                        type="text"
                        value={bulkAddForm.courierNo}
                        onChange={(e) =>
                          setBulkAddForm((prev) => ({
                            ...prev,
                            courierNo: e.target.value,
                          }))
                        }
                      ...
                      <div className="lg:w-36">
                        {(() => {
                          const stockQuantity = Number(
                            selectedBulkAddInventoryItem?.quantity || 0,
                          );
                          const isOutOfStock =
            ...
            <div className="mt-4">
              <label className="block text-sm text-slate-700">Courier No</label>
              <input
                type="text"
                value={currentItem?.courierNo || ""}
                onChange={(e) =>
                  setCurrentItem((p) => ({ ...p, courierNo: e.target.value }))
                }
                className="border bg-white border-slate-200 rounded-xl p-2 w-full mt-1 text-slate-900 outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-200"
                placeholder="Enter courier no"
              />
            </div>
            ...
            <div className="mt-4">
              {(() => {
                const stockQuantity = Number(
                  selectedEditInventoryItem?.quantity || 0,
                );
                const isOutOfStock =
                  currentItem?.receivedId &&
                  !hasConfiguredVariants(currentItem?.variantRows) &&
              ...
              <div className="mt-4 grid grid-cols-1 gap-4 sm:grid-cols-2">
                ...
                <div>
                  ...
                  <Select
                    ...
                    value={
                      courierStatusOptions.find(
                        (option) =>
                          option.value ===
                          normalizeCourierStatus(currentItem.courierStatus),
                      ) || courierStatusOptions[0]
                    }
                    onChange={(selected) =>
                      setCurrentItem((p) => ({
                        ...p,
                        courierStatus: normalizeCourierStatus(selected?.value),
                      }))
                    }
      ...
      <Modal
        isOpen={isAddOpen}
        onClose={closeAdd}
        title="Add New Courier No Entry"
        maxWidth="max-w-2xl"
      >
        <form
                  ...
                  <table className="min-w-[900px] w-full text-left text-sm">
                    <thead className="bg-slate-50 text-xs font-bold uppercase tracking-wider text-slate-500">
                      <tr>
                        ...
                        <th className="px-4 py-3">Courier No</th>
                        ...
                        <tr key={`${item.label}-${itemIndex}`}>
                          ...
                          <td className="px-4 py-4 align-top">
                            <input
                              ...
                              className="h-10 w-full min-w-[160px] rounded-xl border border-slate-200 bg-white px-3 text-sm font-medium text-slate-900 outline-none transition focus:border-indigo-500 focus:ring-4 focus:ring-indigo-500/10"
                              placeholder="Enter courier no"
                            />
                          </td>
                          <td className="px-4 py-4 align-top">
                                  ...
                                  >
                                    ...
                                    <div>
                                      ...
                                      {(() => {
                                        ...
                                        const match = getVariantDisplayRows(
                                          invItem,
                                        ).find(
                                          (v) =>
                                            String(v.size || "") ===
              ...
              <Select
                ...
                value={
                  courierStatusOptions.find(
                    (option) =>
                      option.value ===
                      normalizeCourierStatus(createForm.courierStatus),
                  ) || courierStatusOptions[0]
                }
                onChange={(selected) =>
                  setCreateForm((prev) => ({
...
export default CourierNoEntryTable;
