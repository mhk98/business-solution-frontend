...
} from "../../features/damageReturn/damageReturn";
...
const DamageReturnTable = () => {
  ...
  const { data: damageStockRes } = useGetAllDamageStockWithoutQueryQuery();
  const damageStockData = damageStockRes?.data || [];
  ...
  const { data: responseData, isLoading } = useGetAllDamageReturnQuery(queryParams);
  const [insertDamageReturn, { isLoading: isCreating }] = useInsertDamageReturnMutation();
  const [deleteDamageReturn] = useDeleteDamageReturnMutation();
  ...
  const selectedStock = useMemo(
    () => damageStockData.find((item) => String(item.Id) === String(createForm.receivedId)),
    [damageStockData, createForm.receivedId],
  );
  ...
  const handleStockSelect = (option) => {
    ...
    const found = damageStockData.find((s) => String(s.Id) === String(option.value));
    setCreateForm((prev) => ({
      ...prev,
  ...
  const handleCreateSubmit = async (e) => {
    ...
    if (!createForm.receivedId) {
      toast.error("Please select a damage stock item");
      return;
    }
    ...
    if (selectedStock && qty > Number(selectedStock.quantity || 0)) {
      toast.error(`Quantity cannot exceed available damage stock (${selectedStock.quantity})`);
      return;
    }

    try {
      await insertDamageReturn({
      ...
      toast.success("Damage Return created successfully");
      closeAdd();
    } catch (err) {
      toast.error(err?.data?.message || err?.message || "Failed to create Damage Return");
    }
  };

  const handleDelete = async (record) => {
    const confirmed = await requestDeleteConfirmation({
      title: "Delete Damage Return?",
      message: `Are you sure you want to delete ${record.name}? This will restore Damage Stock.`,
    });

    if (!confirmed) return;

    try {
      await deleteDamageReturn(record.Id).unwrap();
      toast.success("Damage Return record deleted");
    } catch (err) {
    ...
    >
      <div className="p-6 lg:p-8">
        <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
          <div>
            <h2 className="text-2xl font-bold text-slate-950">Damage Return History</h2>
            <p className="mt-1 text-sm text-slate-500">
              Return damaged products from Damage Stock to supplier
            </p>
          </div>
            ...
            <table className="min-w-full divide-y divide-slate-100">
              ...
              <tbody className="divide-y divide-slate-100 bg-white">
                {isLoading ? (
                  ...
                  <tr>
                    <td colSpan={7} className="px-6 py-12 text-center text-sm text-slate-400">
                      No Damage Return records found
                    </td>
                  </tr>
                )}
              </tbody>
      ...
      {/* Add Modal */}
      <Modal isOpen={isAddOpen} onClose={closeAdd} title="Add Damage Return" maxWidth="max-w-xl">
        <form onSubmit={handleCreateSubmit} className="space-y-4">
          <div>
            <label className="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-1.5">
              Select Item from Damage Stock
            </label>
            <Select
              options={stockDropdownOptions}
              value={stockDropdownOptions.find((o) => o.value === String(createForm.receivedId)) || null}
              onChange={handleStockSelect}
              placeholder="Search & select damage stock item..."
...
export default DamageReturnTable;
