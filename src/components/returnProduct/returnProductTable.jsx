...
const parseSalesReturnItems = (value) => {
...
const getSalesReturnRowItems = (row) => {
  const items = parseSalesReturnItems(row?.items);
  return items.length ? items : [row];
};

const getSalesReturnItemsTotalQuantity = (items = []) =>
...
const ReturnProductTable = () => {
  ...
  const getSourceRecordForSalesReturnItem = (item) => {
  ...
  const getItemUnitPrice = (item, field) => {
    const sourceRecord = getSourceRecordForSalesReturnItem(item);
  ...
  const handleDelete = async (id) => {
    const confirmed = await requestDeleteConfirmation({
      title: "Delete sales return product?",
      message:
        "This sales return product entry will be removed permanently. This action cannot be undone.",
    });
    if (!confirmed) return;
  ...
  return (
    ...
    >
      ...
      <div className="my-2 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        ...
        <div className="flex items-center justify-between sm:justify-end gap-3 rounded-xl border border-slate-200 bg-white px-4 py-2 shadow-sm">
          <div className="flex items-center gap-2 text-slate-700">
            ...
            <span className="text-sm">Total Sales Return</span>
          </div>
      ...
      <div className="return-product-table-scroll four-row-table-scroll mt-6 rounded-2xl border border-slate-200">
        <table className="w-full min-w-[1680px] divide-y divide-slate-200 [&_th]:px-3 lg:[&_th]:px-4 [&_td]:px-3 lg:[&_td]:px-4 [&_th]:py-3 [&_td]:py-3">
          ...
          <tbody className="divide-y divide-slate-200 bg-white">
            {rows.map((rp) => {
              const rowItems = getSalesReturnRowItems(rp);
              const rowTotalQuantity =
                getSalesReturnItemsTotalQuantity(rowItems);
      ...
      <Modal
        isOpen={isAddOpen}
        onClose={closeAdd}
        title="Add New Sales Return Product"
        maxWidth="max-w-2xl"
      >
        <form
