...
} from "../../features/salesDue/salesDue";
...
const SalesDueTable = () => {
  ...
  const { data, isLoading, isError, error, refetch } =
    useGetAllSalesDueQuery(queryArgs);

  useEffect(() => {
    if (isError) console.error("Sales Due fetch error:", error);
    if (!isLoading && data) {
      setRows(data?.data ?? []);
      setTotalPages(
  ...
  const [insertSalesDue] = useInsertSalesDueMutation();
  const [updateSalesDue] = useUpdateSalesDueMutation();
  const [deleteSalesDue] = useDeleteSalesDueMutation();
  const [paySalesDue] = usePaySalesDueMutation();
  ...
  const handleCreate = async (e) => {
    ...
    try {
      const res = await insertSalesDue({
  ...
  const handleUpdate = async () => {
    ...
    try {
      const res = await updateSalesDue({
        id: currentItem.Id,
        data: {
  ...
  const handleDelete = async (id) => {
    ...
    try {
      const res = await deleteSalesDue(id).unwrap();
  ...
  return (
    ...
    >
      <div className="mb-6 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <h2 className="text-lg font-bold text-slate-900">Sales Due</h2>
        {canManage && (
          <button
            type="button"
            onClick={openAdd}
